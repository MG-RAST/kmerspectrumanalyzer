...
def decimate(a, d):  # subroutine to reduce dimensionality
   print "decimate invoke:", np.array(a).shape
   try:
     d2 = d[1]
     d1 = d[0]
   except:
...
if __name__ == '__main__':
    ...
    if opts.xlabel != None: 
        ...
        xlabel = "MP 1 21mer abundance"
    if opts.ylabel != None: 
        ylabel = opts.ylabel
    else:
        ylabel = "MP 2 21mer abundance"
    outfile = opts.outfile
    if opts.outfile == None: 
        outfile = infile

    if opts.verbose: sys.stdout.write("Trying to read %s with numpy...\n"%infile)
